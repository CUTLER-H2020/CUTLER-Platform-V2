...
data "vsphere_datastore" "datastore" {
  name          = "${var.vsphere_ds_100}"
  datacenter_id = "${data.vsphere_datacenter.dc.id}"
}

data "vsphere_host" "host" {
  name          = "${var.vsphere_host_100}"
  datacenter_id = "${data.vsphere_datacenter.dc.id}"
}
  ...
  clone {
    ...
    customize {
      linux_options {
		...
		host_name = "${format("%s%0d", var.c1_name_prefix, count.index)}"
		domain    = "cutler-p4.local"
      }
      network_interface {
        ipv4_address    = "${cidrhost(var.vm_network_address, var.c1_ip_address_start + count.index)}"
        ipv4_netmask    = 24
      }
  ...
  clone {
    ...
    customize {
      linux_options {
                ...
                host_name = "${format("%s%02d", var.c2_name_prefix, count.index)}"
                domain    = "cutler-p4.local"
      }
      network_interface {
        ipv4_address    = "${cidrhost(var.vm_network_address, var.c2_ip_address_start + count.index)}"
        ipv4_netmask    = 24
      }
          ipv4_gateway = "${var.vm_gateway}"
          dns_server_list = ["8.8.8.8","1.1.1.1"]
  ...
  clone {
    ...
    customize {
      linux_options {
                ...
                host_name = "${format("%s%02d", var.c3_name_prefix, count.index)}"
                domain    = "cutler-p4.local"
      }
      network_interface {
        ipv4_address    = "${cidrhost(var.vm_network_address, var.c3_ip_address_start + count.index)}"
        ipv4_netmask    = 24
      }
          ipv4_gateway = "${var.vm_gateway}"
          dns_server_list = ["8.8.8.8","1.1.1.1"]
  ...
  clone {
    ...
    customize {
      linux_options {
                ...
                host_name = "${format("%s%02d", var.c4_name_prefix, count.index)}"
                domain    = "cutler-p4.local"
      }
      network_interface {
        ipv4_address    = "${cidrhost(var.vm_network_address, var.c4_ip_address_start + count.index)}"
        ipv4_netmask    = 24
      }
          ipv4_gateway = "${var.vm_gateway}"
          dns_server_list = ["8.8.8.8","1.1.1.1"]
